// ReceiverDashboard.jsx - Dashboard for receivers to view received letters
import React, { useState, useEffect, useMemo } from 'react';
import { motion } from 'framer-motion';
import { useAuth } from '../contexts/AuthContext';
import RoleSwitcher from '../components/RoleSwitcher';

export default function ReceiverDashboard({ onSwitchToSender }) {
  ...
  // Handle letter click - navigate to letter viewer which will show OptionsPage after viewing
  const handleLetterClick = (letter) => {
    if (letter.token || letter.originalToken) {
      const token = letter.token || letter.originalToken;
      // Navigate to letter viewer - after viewing, user will see OptionsPage
      window.location.href = `/letter/${token}`;
    } else {
      console.warn('Letter has no token, cannot navigate');
    }
  };
            ...
            {receivedLetters.map((letter, index) => (
              ...
              >
                ...
                <p className="text-gray-600 font-serif text-sm line-clamp-2">
                  {letter.introductory || letter.content || 'Click to read...'}
                </p>
              </motion.div>
            ))}
          </div>
